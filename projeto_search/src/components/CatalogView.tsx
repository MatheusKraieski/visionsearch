    ...
    <div className="max-w-[1280px] mx-auto px-4 md:px-6 py-6 md:py-8">
      <div className="flex items-start justify-between mb-6 md:mb-8">
        <div>
          ...
          <p className="hidden sm:block text-sm text-slate-500">Navegue, filtre por categoria ou busque por nome, SKU ou tag.</p>
        </div>
      ...
      <div className="card p-3 mb-6 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 md:py-2 bg-slate-50 border border-slate-200 rounded-md text-sm focus:outline-none focus:border-[var(--accent)] focus:bg-white focus:ring-2 focus:ring-[var(--accent-ring)]"
          />
        </div>

        <div className="flex items-center gap-1 bg-slate-50 border border-slate-200 rounded-md p-1 overflow-x-auto flex-shrink-0">
          {[{ id: 'all', label: 'Todos', count: allProducts.length }, ...CATEGORIES.map((c) => ({ ...c, count: countByCategory(c.id) }))].map((c) => (
            <button
              key={c.id}
              onClick={() => setCat(c.id)}
              className={`flex-shrink-0 px-3 py-1.5 rounded text-xs font-semibold transition-colors ${cat === c.id ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
            >
              {c.label}
